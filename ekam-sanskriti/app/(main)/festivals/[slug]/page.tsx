      ...
      <div className="max-w-7xl mx-auto px-4 md:px-12 -mt-8 relative z-20">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-2 flex overflow-x-auto hide-scrollbar">
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-bold text-sm whitespace-nowrap transition-colors ${activeTab === 'watch' ? 'bg-red-50 text-red-700' : 'text-gray-700 hover:bg-gray-50 hover:text-gray-900'}`}
          >
            <PlayCircle size={18} /> 🎥 {tabDict.watch}
          </button>
          <button 
            onClick={() => setActiveTab('story')}
            className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-bold text-sm whitespace-nowrap transition-colors ${activeTab === 'story' ? 'bg-red-50 text-red-700' : 'text-gray-700 hover:bg-gray-50 hover:text-gray-900'}`}
          >
            <ScrollText size={18} /> 📝 {tabDict.storySignificance}
          </button>
          <button 
            onClick={() => setActiveTab('celebration')}
            className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-bold text-sm whitespace-nowrap transition-colors ${activeTab === 'celebration' ? 'bg-red-50 text-red-700' : 'text-gray-700 hover:bg-gray-50 hover:text-gray-900'}`}
          >
            <PartyPopper size={18} /> 🎉 {tabDict.howCelebrated}
          </button>
          <button 
            onClick={() => setActiveTab('wiki')}
            className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl font-bold text-sm whitespace-nowrap transition-colors ${activeTab === 'wiki' ? 'bg-red-50 text-red-700' : 'text-gray-700 hover:bg-gray-50 hover:text-gray-900'}`}
          >
                          ...
                          {data.celebration.rituals.map((ritual: string, i: number) => (
                            <li key={i} className="flex items-start gap-4 p-4 rounded-2xl bg-gray-50 hover:bg-orange-50 transition-colors border border-transparent hover:border-orange-100 group">
                              ...
                              <button 
                                ...
                                className={`flex items-center justify-center w-8 h-8 rounded-full transition-colors flex-shrink-0 opacity-0 group-hover:opacity-100 ${playingAudioId === `ritual-${i}` ? 'bg-red-100 text-red-600 opacity-100' : 'bg-white text-gray-700 hover:bg-gray-100 shadow-sm'}`}
                              >
